...
class CompanyController extends Controller
{
    ...
    public function companyProfile()
    {
        ...
        $company = Company::findOrFail(Auth::guard('company')->user()->company->id);
    ...
    public function updateCompanyProfile(CompanyFrontFormRequest $request)
    {
        ...
        $company->website = (false === strpos($website, 'https')) ? 'https://' . $website : $website;
        ...
        if (!empty($company->email)) {
            Subscription::where('email', $company->email)->delete();
    ...
    {
        $jobs = Auth::guard('company')->user()->company->jobs()->paginate(10);
    ...
    {
        $company_id = Auth::guard('company')->user()->company->id;
    ...
    public function companyPackages()
    {
        ...
        $existing_package = $company->package_id > 0
            ? Package::findOrFail($company->package_id)
            : null;

        //$currentSequence = $existing_package ? $existing_package->package_sequence : 0;

        $packages = Package::where('package_for', 'employer')
            ->where('package_sequence', '>', 1) // only upgrades
            ->orderBy('package_sequence')
            ->get();

        return view('company_packages', [
            'packages' => $packages,
    ...
    {
        $data = array();
        $unlocked_users = Unlocked_users::where('company_id', Auth::guard('company')->user()->company->id)->first();
    ...
    {
        $cvsSearch = Auth::guard('company')->user()->company;
        if ($cvsSearch && $cvsSearch->availed_cvs_quota < $cvsSearch->cvs_quota) {
            ...
            $unlock = Unlocked_users::where('company_id', Auth::guard('company')->user()->company->id)->first();
            if (null !== ($unlock)) {
                ...
                $unlock->company_id = Auth::guard('company')->user()->company->id;
                $unlock->unlocked_users_ids = $newString;
                $unlock->save();
            }
