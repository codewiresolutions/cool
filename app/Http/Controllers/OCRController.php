...
class OCRController extends Controller
{
    ...
    public function ResumeOCRPost(Request $request)
    {
        ...
        if ($request->hasFile('file')) {
            ...
            $request->file('file')->storeAs('public/cv', "$hash.$ext");
            ...
            $pdfUrl = url("storage/cv/$hash.pdf");

            // Send to Superparser
            $data = ['resume_url' => $pdfUrl];
            $payload = json_encode($data);

            $curl = curl_init();
            $apiUrl = trim(env('SUPER_PARSER_URL'));
            $apiKey = trim(env('SUPER_PARSER_KEY'));
            ...
            if (curl_errno($curl)) {
                return back()->withErrors(["Curl error: " . curl_error($curl)]);
            }
            ...
            $parsedData = $parsedData['data'] ?? null;
            // dd($parsedData);

            if ($parsedData === null) {
                return back()->withErrors(["Something went wrong while fetching data. Please try again later."]);
            }
            ...
            return view('ocr.resume-ocr')->with(compact(['parsedData', 'majorSubjects', 'countries', 'resultTypes', 'degreeLevels']));
        }
