...
class MajorSubjectController extends Controller
{
    ...
    public function storeMajorSubject(MajorSubjectFormRequest $request)
    {
        ...
        if (MajorSubject::where('major_subject', $request->input('major_subject'))->where('degree_level_id', $request->input('degree_level_id'))->exists()) {
            $degree_level = DegreeLevel::find($request->input('degree_level_id'));
            
            return back()->withInput()->withErrors(
                [
                    $request->input('major_subject').' already exists against '.$degree_level->degree_level
                ]);
        }
        ...
        $majorSubject->degree_level_id = $request->input('degree_level_id');
