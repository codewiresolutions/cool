...
trait ProfileLanguageTrait
{
    ...
    {
        $checkLanguageExists = ProfileLanguage::where('user_id', $user_id)->where('language_id', $request->input('language_id'))->count();
        if( $checkLanguageExists == 0 ) {
            $profileLanguage = new ProfileLanguage();
            $profileLanguage = $this->assignLanguageValues($profileLanguage, $request, $user_id);
            $profileLanguage->save();
            ...
            return response()->json(array('success' => false, 'status' => 200, 'html' => "Language already exists in your profile"), 200);
        }
    }
    ...
    {
        $checkSkillExists = ProfileLanguage::where('user_id', $user_id)->where('id','!=', $profile_language_id)->where('language_id', $request->input('language_id'))->count();
        if( $checkSkillExists == 0 ) {
            $profileLanguage = ProfileLanguage::find($profile_language_id);
            $profileLanguage = $this->assignLanguageValues($profileLanguage, $request, $user_id);
            $profileLanguage->update();
