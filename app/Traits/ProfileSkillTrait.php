...
trait ProfileSkillTrait
{
    ...
    {
        $checkSkillExists = ProfileSkill::where('user_id', $user_id)->where('job_skill_id', $request->input('job_skill_id'))->count();
        if( $checkSkillExists == 0 ) {
            ...
            return response()->json(array('success' => false, 'status' => 200, 'html' => "Skill already exists in your profile"), 200);
        }
    }
    ...
    {
        $checkSkillExists = ProfileSkill::where('user_id', $user_id)->where('id','!=', $skill_id)->where('job_skill_id', $request->input('job_skill_id'))->count();
        if( $checkSkillExists == 0 ) {
            $profileSkill = ProfileSkill::find($skill_id);
            $profileSkill->user_id = $user_id;
            $profileSkill->job_skill_id = $request->input('job_skill_id');
