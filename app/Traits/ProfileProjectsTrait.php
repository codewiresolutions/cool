...

{
    ...
    public function showFrontProfileProjects(Request $request, $user_id)
    {
        ...
        if (isset($user) && count($user->profileProjects)):
            ...
            foreach ($user->profileProjects as $project):
                ...
                $date_end = $project->is_on_going == 1
                    ? 'Currently ongoing'
              ...
              <div class="col-md-4" id="project_' . $project->id . '">
                ...
                if (!empty($techStack)) {
                    $html .= '<p><strong>Technologies:</strong> ' . $techStack . '</p>';
                }

                $html .= '<a class="text text-default" href="javascript:void(0);" onclick="showProfileProjectEditModal(' . $project->id . ')">' . __('Edit') . '</a>&nbsp;|&nbsp;
                      <a class="text text-danger" href="javascript:void(0);" onclick="delete_profile_project(' . $project->id . ')">' . __('Delete') . '</a>
                    </div>
                </div>
              </div>
              <!--Project End-->';
...

    {
        ...
        $profileProject->tech = $request->input('tech');
