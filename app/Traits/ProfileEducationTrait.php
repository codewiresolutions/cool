...
trait ProfileEducationTrait
{
    ...
    {
        $user = User::with(['profileEducation.profileEducationMajorSubjects'])->find($user_id);

        $html = '<div class="panel-group">';

        if ($user && $user->profileEducation->count()):
            foreach ($user->profileEducation as $education):

                $majorSubjectIds = $education->getProfileEducationMajorSubjectsArray();
                $majorSubjectNames = '';

                if (!empty($majorSubjectIds)) {
                    $subjects = MajorSubject::findMany($majorSubjectIds);
                    $majorSubjectNames = $subjects->pluck('major_subject')->implode(', ');
                }
                $html .= '<div class="panel panel-info" id="education_' . $education->id . '">
                        <div class="panel-heading">
                            <h4>' . $education->getDegreeLevel('degree_level') . ' - ' . e($majorSubjectNames) . '</h4>
                        </div>
                        <div class="panel-body">
                            <p class="text-left"><h5>' . e($education->degree_title) . '</h5></p>
                            <p class="text-left">' . e($education->date_completion) . ' | ' . e($education->getCity('city')) . '</p>
                            <p class="text-left">' . e($education->institution) . '</p>
                        </div>
                        <div class="panel-footer">
                            <a href="javascript:void(0);" onclick="showProfileEducationEditModal(' . $education->id . ',' . $education->state_id . ',' . $education->city_id . ',' . $education->degree_type_id . ');" class="text text-default">' . __('Edit') . '</a>
                            &nbsp;|&nbsp;
                            <a href="javascript:void(0);" onclick="delete_profile_education(' . $education->id . ');" class="text text-danger">' . __('Delete') . '</a>
                        </div>
                    </div>';
            endforeach;
        endif;
