...

{
    ...
    public static function countNumJobs($field = 'title', $value = '')
    {
        if (!empty($value)) {
            ...
            if ($field == 'career_level_id') {
                return DB::table('jobs')->where('career_level_id', '=', $value)->where('is_active', '=', 1)->where('expiry_date', '>', Carbon::now())->count('id');
            }
...

    {
        ...
        if ($request->input('current_location_id') != 'all') {
            $job->current_location_id = $request->input('current_location_id');
        }
        return $job;
