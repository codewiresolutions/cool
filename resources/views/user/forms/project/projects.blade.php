...
<div class="row">
    <div class="col-md-12">
        <div class="formrow">
            <label for="" class="d-block" style="font-weight: bold; font-size: 18px;">
                {{ __('Your Projects') }}
            </label>
            <div id="projects_div" class="mt-2"></div>
            <div class="text-right mt-3">
                <a href="javascript:;" class="btn btn-success" onclick="showProfileProjectModal();">
                    {{ __('Add Project') }}
                </a>
            </div>
        </div>
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        function showProjects() {
            ...
            $.post("{{ route('show.front.profile.projects', $user->id) }}", {
                user_id: {{$user->id}},
                _method: 'POST',
                _token: '{{ csrf_token() }}'
            })
        ...
        function delete_profile_project(id) {
            ...
            if (confirm(msg)) {
                ...
                $.post("{{ route('delete.front.profile.project') }}", {
                    id: id,
                    _method: 'DELETE',
                    _token: '{{ csrf_token() }}'
                })
