            ...
            <div class="col-md-9 col-sm-9">
                <form action="{{route('job.list')}}" method="get">
                    <div class="searchform row custom_top_margin_second_header">
                        ...
                        <select name="country_id[]" class="form-control" style="background-color: #0096ff; color:#495057">
                    ...
                    <div class="col-lg-2 pl-lg-0">
                        <select name="job_experience_id[]" class="form-control find_job" style="background-color: #0096ff; color:#495057">
...
$('.btn-job-alert').on('click', function() {
    ...
    @endif
     ...
     $(document).ready(function ($) {
        ...
        var $viewMore = $(this);
        var $viewLess = $viewMore.next('.view_less');
        var $list = $($viewMore.data('target'));

        // Initial check to hide the "View More" button if the content is not overflowing
        if ($list.height() > 100) {
            $list.addClass('hide_vm_ul');
            $viewMore.show();
            $viewLess.hide();
        } else {
            $viewMore.hide();
        ...
        $viewMore.on('click', function (e) {
            e.preventDefault();
            $list.removeClass('hide_vm_ul').find('.more-skill-item').slideDown();
            $(this).hide(); // Hide "View More"
            $viewLess.show(); // Show "View Less"
        });

        // Handle the "View Less" button click
        $viewLess.on('click', function (e) {
            e.preventDefault();
            $list.addClass('hide_vm_ul').find('.more-skill-item').slideUp();
            $(this).hide(); // Hide "View Less"
            $viewMore.show(); // Show "View More"
        });
    });
